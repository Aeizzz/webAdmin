				...
				<tr>
					...
					<td>
						<c:if test="${empty task.assignee}">
							<a href="${ctx}/act/task/claimForm?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInstId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&formFlag=0&assignee=${task.assignee}">任务签收</a>
							<%-- <a href="javascript:claim('${task.id}');">签收任务</a> --%>
						</c:if>
						<c:if test="${not empty task.assignee}">
							<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInstId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&formFlag=0">任务办理</a>
						</c:if>
