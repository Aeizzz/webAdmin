	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="act">
				...
				<tr>
					<td>
						<%--<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInstId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&formFlag=1">${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}</a>--%>
					...
					<td>
						<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInstId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&formFlag=1">详情</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
