...
public class ActProcessService extends BaseService {
	 ...
	 * 流程定义列表
	 */
	public Page<ProcessInstance> runningList(Page<ProcessInstance> page, String procInstId, String procDefKey) {

	    ProcessInstanceQuery processInstanceQuery = runtimeService.createProcessInstanceQuery();

	    if (StringUtils.isNotBlank(procInstId)){
		    processInstanceQuery.processInstanceId(procInstId);
	    }
	 ...
	 * 流程定义列表
	 */
	public Page<Act> runningList1(Page<Act> page, String procInstId, String procDefKey) {
		...
		page.setCount(processInstanceQuery.count());
		List<ProcessInstance> list = processInstanceQuery.listPage(page.getFirstResult(), page.getMaxResults());
		List<Act> listAct = new ArrayList<Act>();
		for(int i=0;i<list.size();i++){
			Act act = new Act();
			act.setProcIns(list.get(i));
			listAct.add(act);
		}
		page.setList(listAct);
		return page;
	}
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
	 ...
	 */
	@Transactional(readOnly = false)
	public void deleteProcIns(String procInstId, String deleteReason) {
	 ...
	 */
	@Transactional(readOnly = false)
	public void deleteProc(String procInstId) {
